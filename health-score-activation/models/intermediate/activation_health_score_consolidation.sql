-- Activation Health Score – Consolidation (Intermediate Model)
--
-- Defines the cohort (base_restaurants) and combines Setup, Effort, and Value
-- bucket metrics into total_score, Key, and activation_bucket.
-- This file inlines the full bucket logic so it can be run standalone (e.g. in Metabase).
-- For a single-execution version with the same logic, use marts/activation/mart_activation_health_score.sql.
--
